...
INSERT INTO setting (setting_name, setting_value, setting_description)
SELECT 'paired', '{"paired":false}', 'Whether the app currently considers a device paired, written by AppDataStore.recordPaired(_:). Set true when the app starts already knowing it is paired and whenever pairing completes/reconnects; set false when the device is factory-reset (forgotten) or the connection is lost (see ApplicationDelegate onPairingChange / startup / handleDeviceDisconnect). Seeded false; the app corrects it on launch. A test/observer gates its "is the device connected now?" check on this reading true -- if it is false the device needs (re)pairing (see Tests/00-test-setup.md). This is volatile connection state; the durable "which device, and does the user want to be paired to it" lives in paired_device.'
WHERE NOT EXISTS (SELECT 1 FROM setting WHERE setting_name = 'paired');

INSERT INTO setting (setting_name, setting_value, setting_description)
SELECT 'paired_device', '{"wants":false}', 'The device the app is bonded to, and whether the user wants to be bonded at all. wants is the pairing intent, restored at launch to decide whether to attempt a connection; name is the remembered device name shown while disconnected; uuid is the CoreBluetooth peripheral identifier used to reconnect to the same device rather than rediscovering. Written by AppDataStore.recordWantsPairing(_:) and recordPairedDevice(name:uuid:). Kept apart from the paired row because all three are durable -- they change only when the user pairs or forgets a device -- whereas paired is volatile and cleared on every transient disconnect, which would otherwise lose the intent across a quit while the device is out of range. Seeded with wants false and no device; name and uuid appear on first pairing.'
