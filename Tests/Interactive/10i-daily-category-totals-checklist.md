...
```
- [ ] **(Claude)** Step 3: Capture both stickered faces' category assignments so Scenario B can restore them.
```toml step
[[actions]]
action = "sql_query"
...
- [ ] **(Claude)** Step 1: Point both **Meeting** (face 2) and **Break** (face 8) at one category, and restart so the app picks it up.
...
action = "ensure_unlocked_unpaused"
```
- [ ] **(Claude)** Step 2: Capture which face the cube is resting on, and name the one to flip to.
...
```
- [ ] **(You)** Step 3: Flip the cube to the face named in the prompt, then leave it resting there.
...
timeout_seconds = 0
```
- [ ] **(Claude)** Step 4: Let a minute of real time accrue on this face.
...
command = "sleep 60"
```
- [ ] **(Claude)** Step 5: Pause the device and read the exact combined figure.
...
```
- [ ] **(Claude)** Step 6: Compute what the shared category has recorded today from `time_entry`.
...
```
- [ ] **(Claude)** Step 7: Confirm the menu bar's figure is that whole-category total.
...
expect = "matches"
```
- [ ] **(You)** Step 8: Confirm the duration never dropped when the cube was flipped.
...
- [ ] **(Claude)** Step 1: Restore both faces' original categories and resume the device.
```toml step
[[actions]]
use = "method-3"
...
action = "ensure_unlocked_unpaused"
```
- [ ] **(Claude)** Step 2: Confirm both faces read their original categories again.
```toml step
action = "sql_query"
