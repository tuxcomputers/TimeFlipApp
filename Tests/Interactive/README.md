...
gesture unverified via script (see `../CLAUDE.md`'s "Driving the app directly"), or multiple
elements that must be confirmed changing in lockstep with each other -- a claim two screenshots
can't establish on their own (see "Screenshot-based visual confirmation" in `../CLAUDE.md`).
