...
### Last run - 2026-08-10 13:58 on the branch 'feature/manualMode'
...
- [x] **(Claude)** Step 1: Confirm the device shows connected
...
```
- [x] **(Claude)** Step 2: Note the current max `event_number`
...
```
- [x] **(Claude)** Step 3: Confirm the device isn't locked
...
```
- [x] **(You)** Step 4: Flip to whichever of **Break**/**Meeting** the device is *not* already on
...
poll_interval = 2
```
- [x] **(Claude)** Step 5: Confirm a new `device_event` row exists with a higher `event_number`.
...
expect = "1"
```
- [x] **(Claude)** Step 6: Confirm the menu bar updated to the new face
...
```
- [x] **(You)** Step 3: Turn **off** Bluetooth on the Mac
...
poll_interval = 5
```
- [x] **(You)** Step 4: Flip between **Break** and **Meeting** 2-3 times.
...
```
- [x] **(You)** Step 5: Turn Bluetooth back **on**
