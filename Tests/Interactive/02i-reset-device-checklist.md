...
- [x] **(Claude)** Confirm `db_type` reads `{"type":"test"}` and the device shows connected before
      asking for the flip below. (Confirmed: `{"type":"test"}`, connected, unlocked.)
```toml step
[[actions]]
action = "sql_query"
...
- [x] **(You)** Confirm you flipped the device to a different facet. (Confirmed: new event
      `event_number=15`, facet 8.)
```toml step
...
poll_interval = 2
```
- [x] **(Claude)** Query `device_event` (Method: Read debug output, `../Methods.md` -- by
...
- [x] **(Claude)** Query `debug_log` for the `history`-tagged fetch that picked up this new event
      ...
      logged evidence, not just the queried row. (Confirmed: `"history fetch triggered:
      trigger=live_event known_max=14"` -> `"cheap check device_last_event=15 known_max=14"` ->
      `"dev-check: device_event max_start_epoch OK"`.)
```toml step
action = "wait_for_sql"
