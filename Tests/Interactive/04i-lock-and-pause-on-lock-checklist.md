...
Run **after** `Tests/Bench/04b-lock-and-pause-on-lock-checklist.md`, which covers the same
...
single/double-click-right-half gesture (Method: Status-item click gesture -- not automatable,
`../Methods.md`), or a physical facet flip. Everything else in these scenarios that *can* be
Claude-driven (menu clicks -- Method: Click a status-item menu item; reading menu/badge/icon state
-- Method: Screenshot-based visual confirmation, Read a label or value via accessibility; or a
DB-based running/paused check in place of watching the menu bar, all `../Methods.md`) has been.
...
- [ ] **(Claude)** Confirm the menu bar shows no lock badge and a play icon; if it doesn't, click
...
- [ ] **(Claude)** Confirm no new `is_paused = 1` row was added to `device_event` -- Lock alone,
...
- [ ] **(Claude)** Confirm the menu bar shows no lock badge and a play icon before asking for the
