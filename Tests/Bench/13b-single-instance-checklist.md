...
expect = '{"type":"test"}'
```
- [x] Step 2: Confirm exactly one instance is running, and note which one it is.
...
- [x] Step 1: Launch the app again in the **foreground** and confirm it says it is exiting.
...
```
- [x] Step 2: Confirm there is **still** exactly one instance.
```toml step
action = "shell"
command = "pgrep -f 'TimeFlip[.]app/Contents/MacOS/TimeFlip' | wc -l | tr -d ' '"
expect = "1"
```
- [x] Step 3: Confirm the survivor is the **original** process, not a replacement.
...
```
- [x] Step 4: Confirm the incumbent still holds the device.
...
- [x] Step 1: Quit and relaunch, and confirm the new instance reaches the device.
...
timeout_seconds = 60
```
- [x] Step 2: Confirm exactly one instance is running, and that it is a **new** process.
