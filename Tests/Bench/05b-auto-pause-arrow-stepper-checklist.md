...
`Tests/Interactive/05i-auto-pause-arrow-stepper-checklist.md`, run after this one.
...
- [ ] Note the latest `debug_log_id`. Type three distinct values into the field in quick
      ...
      `1`, `15`, `150`.)
- [ ] Query `debug_log` (tag `auto-pause`) for rows newer than the noted ID and confirm a
...
- [ ] Confirm `auto_pause_minutes` already reads `{"minutes":150}` immediately (before the 1s
      debounce elapses) -- the DB write never waited on the debounce. (Confirmed.)
- [ ] Wait about 1.5s, then query `debug_log` again and confirm exactly **one**
...
- [ ] Type `0` into the field (single change, not part of a rapid sequence) and confirm after ~1.5s
