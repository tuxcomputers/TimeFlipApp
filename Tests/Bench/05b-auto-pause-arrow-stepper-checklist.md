...
### Last run - 2026-08-10 17:23 on the branch 'bugfix/pairingProbeSettle'
...
expect = "{\"type\":\"test\"}"
```
- [x] Step 2: Open Preferences and switch to the Device tab (selected by name).
...
- [x] Step 1: Type `4` directly into the auto-pause text field
...
timeout_seconds = 30
```
- [x] Step 2: Type `26` into the field
...
timeout_seconds = 30
```
- [x] Step 3: Type `0` into the field
...
```
- [x] Step 2: Confirm **each** intermediate value logged a changed + saved-to-DB pair, in order.
...
timeout_seconds = 30
```
- [x] Step 3: Confirm `auto_pause_minutes` already reads `{"minutes":150}`
...
expect = "{\"minutes\":150}"
```
- [x] Step 4: Cconfirm one debounced triggered line and its confirmation.
...
timeout_seconds = 30
```
- [x] Step 5: Type `0` into the field
...
```
- [x] Step 2: Click and hold the **up** arrow
...
hold_seconds = 4
```
- [x] Step 3: Confirm the hold stepped by single digits, then by 5 past the second gridline.
...
```
- [x] Step 2: Click and hold the **down** arrow
...
hold_seconds = 4
```
- [x] Step 3: Query `debug_log` and confirm the sequence mirrors Scenario C
...
```
- [x] Step 4: Reopen Preferences (Device tab) and click the up arrow once, not held.
...
timeout_seconds = 30
```
- [x] Step 5: Close the Settings window
