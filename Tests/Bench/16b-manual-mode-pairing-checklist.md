...
expect = '{"type":"test"}'
```
- [ ] Step 2: Capture the real PIN from `config.json`, so it can be put back.
...
capture = "real_pin"
```
- [ ] Step 3: Confirm the app is paired and reaching the cube right now.
...
expect = "123457"
```
- [ ] Step 3: Capture the log baseline, launch, and wait for the offer.
...
timeout_seconds = 120
```
- [ ] Step 4: Answer the offer with **Switch to Manual Mode**, and confirm a session starts.
...
timeout_seconds = 60
```
- [ ] Step 5: Pick a category on the Faces tab, so there is a clock to lose.
...
timeout_seconds = 30
```
- [ ] Step 6: Confirm the manual segment is open and running.
...
expect = "0"
```
- [ ] Step 7: Click **Forget Device** on the Device tab, and confirm the pairing is gone.
...
timeout_seconds = 30
```
- [ ] Step 8: Confirm the clock is still running, and still says so.
...
- [ ] Step 9: Confirm the scan controls have taken the place of Forget and Reset.
...
```
- [ ] Step 10: Close the Settings window, so the next scenario opens its own.
...
- [ ] Step 1: Put the real PIN back, drop any pairing, and relaunch.
...
use = "method-2"
```
- [ ] Step 2: Confirm it went straight into a manual session.
...
- [ ] Step 3: Confirm nothing was asked.
...
expect = "0"
```
- [ ] Step 4: Confirm the dropdown's pause control is live with nothing paired.
...
```
- [ ] Step 5: Pick a category and confirm the clock records against it.
...
- [ ] Step 1: Scan from the Device tab, and confirm the cube is listed.
...
timeout_seconds = 60
```
- [ ] Step 2: Note the open manual row, then click the discovered row to pair.
...
timeout_seconds = 120
```
- [ ] Step 3: Confirm the factory default was still presented first.
...
expect = "ok"
```
- [ ] Step 4: Confirm the session was stood down and its segment closed off.
...
timeout_seconds = 60
```
- [ ] Step 5: Confirm the app is now timing from the cube.
...
- [ ] Step 1: Close the Settings window.
[Method: Number 23](../Methods.md#method-23).
```toml step
use = "method-23"
```
- [ ] Step 2: Confirm `config.json` holds the real PIN.
...
expect = "$real_pin"
```
- [ ] Step 3: Leave the device unlocked and unpaused.
