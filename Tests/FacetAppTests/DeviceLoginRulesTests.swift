...
final class DeviceLoginRulesTests: XCTestCase {
    ...
    func testAReconnectWithNoStoredPINStillHasSomethingToTry() {
        XCTAssertEqual(DeviceLoginRules.reconnectCandidates(stored: []), ["000000"])
    ...
    func testTwoStoresThatDisagreeArePresentedNewestFirst() {
        // The one case with three candidates, and it has a known cause: a Keychain write that failed put the PIN in
        // the config file instead, so the file holds what the cube took and the Keychain what it was on before.
        XCTAssertEqual(
            DeviceLoginRules.reconnectCandidates(stored: ["654321", "123456"]),
            ["654321", "123456", "000000"]
        )
    }

    func testTwoStoresThatAgreeAreStillOneCandidate() {
        // Every candidate costs a whole connect, so the ordinary case must not pay for the exceptional one.
        XCTAssertEqual(DeviceLoginRules.candidates(stored: ["123456", "123456"]), ["000000", "123456"])
    }
