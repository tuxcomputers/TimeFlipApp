...
@Suite @MainActor
final class FaceColourRulesTests {
    ...
    @Test func testTheCommandIsTheVendorsEightBytes() {
        ...
        #expect(
            FaceColourRules.command(for: wanted(3, "#ff0000")) == Data([0x11, 3, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00])
        )
        #expect(
            FaceColourRules.command(for: wanted(12, "#0000ff")) == Data([0x11, 12, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF])
        )
    }

    @Test func testEightBitsPerChannelBecomesSixteen() {
        ...
        #expect(FaceColourRules.channels(of: Colour(hex: "#ffffff")).red == 65_535)
        #expect(FaceColourRules.channels(of: Colour(hex: "#808080")).green == 0x8080)
        #expect(FaceColourRules.channels(of: Colour(hex: "#000000")).blue == 0)
    }

    @Test func testAFaceWithNoColourIsSentBlack() {
        ...
        #expect(FaceColourRules.command(for: wanted(5, nil)) == Data([0x11, 5, 0, 0, 0, 0, 0, 0]))
    }

    @Test func testTheFacesAreTheOnesACubeHas() {
        ...
        #expect(FaceColourRules.faces == DeviceFaceRules.reported)
        #expect(!(FaceColourRules.faces.contains(ManualFace.all[0])))
    }

    @Test func testARowSaysBothFormsOfTheColour() {
        ...
        #expect(described == "face 2 Meeting #ff0000 as rgb16 ffff,0000,0000")
    }

    @Test func testARowForAFaceHoldingNothingSaysSo() {
        // Both halves are absent and both are named: a blank would read as a row that failed to fill itself in.
        #expect(
            FaceColourRules.describe(FaceColour(face: 7, categoryName: nil, colour: nil)) == "face 7 no category off as rgb16 0000,0000,0000"
        )
    }
