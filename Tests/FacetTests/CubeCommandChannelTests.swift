...
final class CubeCommandChannelTests {
    ...
    func theDeadlineEndsTheExchangeAndStartsTheNext() {
        let channel = self.channel()
        var first: Bool?
        var second: Bool?
    ...
    @Test("The link ending drops what is queued without answering for it")
    func linkEndedCallsNothing() {
        let channel = self.channel()
        var reported: Bool?
        var answered: DeviceCommandRules.Status??
        ...
        // Neither is told anything. A caller given `false` could not tell a cube that refused from a cube that
        // is not there, and those have opposite remedies.
        #expect(reported == nil)
        #expect(answered == nil)
        #expect(channel.isCommandInFlight == false)
        #expect(channel.scheduledSeconds == nil)

        // And nothing queued survives to go out on the next link.
        let before = wire.transmitted.count
        channel.startNextIfIdle()
        #expect(wire.transmitted.count == before)
    }
}
