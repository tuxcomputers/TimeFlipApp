...
@Suite @MainActor
final class FaceColourSyncTests {
    ...
    @Test func testALinkComingUpSendsEveryFaceInOrder() {
        let wire = Wire()

        sync(on: wire).sendAll(because: "a test")

        #expect(wire.faces == Array(1...12))
    }

    @Test func testTheNextFaceWaitsForTheOneBeforeIt() {
        ...
        #expect(wire.faces == [1], "one command out, eleven still queued")

        wire.waiting.removeFirst()(true)

        #expect(wire.faces == [1, 2], "and the second only once the first came back")
    }

    @Test func testACubeThatRefusesOneFaceIsStillToldAboutTheRest() {
    ...
    @Test func testAnEditSendsOneFaceAndNotTwelve() {
        let wire = Wire()
    ...
    @Test func testAFaceNoCubeHasIsNotSent() {
    ...
    @Test func testAnEditArrivingMidRunGoesOutBehindIt() {
        ...
        #expect(wire.faces == [1], "still just the one that was already out")

        while !wire.waiting.isEmpty { wire.waiting.removeFirst()(true) }

        #expect(wire.faces == Array(1...12) + [1], "and the edit lands after the run it arrived during")
    }

    @Test func testAFaceAlreadyWaitingIsNotQueuedTwice() {
        ...
        #expect(wire.faces == [4, 5])
    }

    @Test func testAFaceRecolouredWhileItsOwnCommandIsOutIsSentAgain() {
        ...
        #expect(wire.faces == [4, 4])
    }

    @Test func testTheColourIsReadWhenTheCommandIsBuiltAndNotWhenItWasQueued() {
        ...
        #expect(wire.sent.first == Data([0x11, 1, 0xFF, 0xFF, 0, 0, 0, 0]), "the one already out was red")
        #expect(wire.sent.last == Data([0x11, 12, 0, 0, 0, 0, 0xFF, 0xFF]), "and the last one read blue")
    }

    // MARK: - the cube asking

    @Test func testACubeBecomingConnectedSendsAllTwelve() {
        let wire = Wire()
    ...
    @Test func testSettlingTwiceOnOneConnectionSendsOnce() {
    ...
    @Test func testSettlingOnACubeNoLongerConnectedSendsNothing() {
    ...
    @Test func testEachNewConnectionIsItsOwnChangeToTrue() {
        let wire = Wire()
        let link = Link()
        let colours = sync(link: link, on: wire)
        colours.linkSettled()
        ...
        #expect(wire.faces.count == 24, "twelve for each connection")
    }

    @Test func testTheCubeAskingIsAnswered() {
        let wire = Wire()
        let colours = sync(on: wire)
        colours.linkSettled()
        ...
        #expect(wire.faces == Array(1...12) + Array(1...12), "the link, then the asking")
    }

    @Test func testTheCubeAskingBeforeTheLinkSettlesIsAnsweredByTheLinkSettling() {
        ...
        #expect(wire.sent == [], "nothing on the wire while the login is still talking")

        colours.linkSettled()

        #expect(wire.faces == Array(1...12), "twelve, not twenty-four")
    }

    @Test func testTheCubeAskingAgainInsideTheCooldownIsNotAnswered() {
        ...
        #expect(wire.faces == Array(1...12) + Array(1...12), "twenty-four, not forty-eight")
    }

    @Test func testAnEditIsNotSilencedByTheCooldown() {
        ...
        #expect(wire.faces.last == 4)
    }

    // MARK: - the link going

    @Test func testWhatIsStillQueuedIsDroppedWhenTheLinkGoes() {
        ...
        #expect(wire.faces == [1], "the one that was already out, and nothing behind it")
    }

    @Test func testANewLinkGetsAFreshHearing() {
        ...
        #expect(wire.faces.count == 48, "two connections and an answered ask on each")
    }

    @Test func testALinkGoingMidCommandDoesNotStallEveryConnectionAfterIt() {
        ...
        colours.linkSettled()
        #expect(wire.faces == [1], "one out, and it is never going to be answered")
        ...
        #expect(wire.faces == [1, 1], "the next connection starts its run rather than sending nothing")
    }

    @Test func testAskingIsNotCarriedOverALinkThatWent() {
        ...
        #expect(wire.faces == Array(1...12), "the next connection sends them, and no extra run on top")
    }
}
