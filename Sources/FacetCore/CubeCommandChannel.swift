...
package final class CubeCommandChannel {
    ...
    /// The deadline in flight, and `nil` while nothing is out.
    ///
    /// **The `TimerHolder` this replaced is gone with the clock port**, and what it bought is worth naming
    /// rather than dropping: it let a `deinit` invalidate the timer, which a `@MainActor` class cannot do to
    /// its own non-Sendable properties. What that saved is one wake-up, the deadline being a one-shot that
    /// captures `self` weakly, so a channel dropped with one outstanding got a callback that found nothing.
    /// `linkEnded` is what really takes a deadline down, and it is explicit.
    ...
    ///
    /// **Private now that the clock is a port**: this was package purely so a test could take the place of the
    /// run loop, and a test drives `Scheduler` instead.
    private func fire() {
        debugLog?.record(
            .command,
            isReadingBack
    ...
    // **There is deliberately no `linkEnded` here**, and the paragraph above is why: this channel's lifetime
    // *is* the link's. `BluetoothRadio` drops the `DeviceLogin` on every path that clears `connectedDevice`,
    // and the channel goes with it, so everything queued is released with its completion uncalled without
    // anybody having to say so. A method that reset it in place would be a second way to do what dropping it
    // already does, and it would have to be reachable from `main.swift` to be called at all, which a
    // per-connection object is not.
    //
    // The modules that *do* need one are the app-lifetime callers holding state across a send, which is what
    // `FaceColourSync.linkEnded`, `HistoryIngestor.linkEnded` and `DeviceSettingsSync.linkEnded` are, and
    // `LinkEndedFanOutTests` is what keeps them wired.
    ...
    private func armDeadline() {
        deadline?.cancel()
        // **Not `mayGroup`.** This is how long the app will wait for a cube before giving up on a command, and
        // a platform free to stretch it is a platform quietly changing that answer.
