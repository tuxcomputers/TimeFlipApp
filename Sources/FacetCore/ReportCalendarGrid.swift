...
/// The month-grid arithmetic behind [ReportCalendar], kept apart from the drawing so the parts that are easy to get
...
enum ReportCalendarGrid {
    ...
    /// Every cell of the grid for the month containing `date`, in order, reading left to right and top to bottom.
    ///
    ...
    ///
    /// The short symbols cut to two characters, which is what the system calendar shows and what keeps the row even.
    /// The obvious alternatives are both worse, and both were measured: `veryShortWeekdaySymbols` gives the ambiguous
    ...
    static func weekdaySymbols(calendar: Calendar, locale: Locale) -> [String] {
        ...
        // shortWeekdaySymbols is Sunday-first regardless of the locale's own first weekday, so rotate it to match the
    ...
    static func month(
        ...
        guard let moved = calendar.date(
            byAdding: .month,
            value: months,
            to: startOfMonth(containing: displayed, calendar: calendar)
        ) else {
            return nil
        }
