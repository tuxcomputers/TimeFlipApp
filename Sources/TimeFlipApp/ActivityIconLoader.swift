...
enum ActivityIconLoader {
    ...
    /// Renders an SVG in `fill` instead of as a template image, so the artwork keeps its own black
    /// line work while the filled areas take the supplied colour. `image(named:pointSize:)` can't do
    ...
    ///
    /// The colour is substituted textually, replacing the placeholder fill the artwork is authored
    /// with. Note AppKit's SVG renderer ignores 8-digit `#RRGGBBAA` and silently falls back to
    /// black, so the replacement is always written as 6-digit hex.
    static func colouredImage(named name: String, pointSize: CGFloat, fill: NSColor) -> NSImage? {
        let trimmed = name.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty,
              let url = resolveURL(for: trimmed),
              let svg = try? String(contentsOf: url, encoding: .utf8),
              let hex = hexString(for: fill) else { return nil }
        let recoloured = svg.replacingOccurrences(of: placeholderFill, with: "fill=\"\(hex)\"")
