...
| What the menu bar **does** while in manual mode | The status item is split down the middle, and the right half's pause and lock have no device to act on | **Built.** Both halves open the menu. The left half briefly opened Settings instead, on the reading that manual timing is driven from there; that was wrong, because Quit lives only in the menu and quitting is the only way *out* of manual mode, with the work being done in a Settings window that has no Quit of its own. Putting the single exit behind knowing which half to click is not a trade worth making for a shortcut. The rule sits ahead of the low-battery route for the same reason -- a stale blink must not divert the left half either. All of it now lives in `MenuBarClickRouter`, out of the AppKit handler no test could call |
