...
- Category assignment **replaced** free-text per-face editing: a face's identity is its `category_id`, and the
  ...
  (`TopFaceEditor`'s `nameBinding`, `SettingsViews.swift`) is gone with the rest of that file.
...
the previous suite's `09b-device-rename` checklist, last run against the cube on 2026-08-02, and what that run
...
`MockEventHTTPServer` are all in the previous implementation; none of them exists in the current app. What carries forward is the
