...
| Change | Why | Decision |
|---|---|---|
| A threshold check on the existing `reconnectAttempt` counter, plus the confirmation prompt itself | This is the entry point the spec calls for -- nothing today compares the counter to anything | Use the existing counter and its capped backoff (`ApplicationDelegate.swift:174`, `scheduleReconnect()`); only the `== 3` comparison and the alert itself are new |
...
| A durable "manual mode is on" setting, surviving a relaunch | Nothing today models "don't try the real device right now" as state that persists; without it, relaunching mid-manual-session goes straight back to retrying the real cube | Not fleshed out |
| How manual mode ends | The spec says ending needs to be at least as deliberate as entering; auto-exit on the next successful connect is called out as probably wrong on its own, since a cube brushing into range for a moment would silently end the mode mid-segment | Not fleshed out |
