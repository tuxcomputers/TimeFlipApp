...
| Which database a launch opened | It decides where every segment of the day lands, and the moment it is worth knowing is the moment you have forgotten which one you started under | **Built.** The `TEST`/`PROD` tag existed but only in developer mode; it now shows on every run, including on the no-device placeholder, which is exactly when the question tends to come up. The developer gate went with the flag on 2026-09-04, which changed nothing on screen: the flag was always on. `PROD` moved from white to `.labelColor` -- white only ever read against a dark menu bar, and a tag shown in every run has to survive light appearance too |
