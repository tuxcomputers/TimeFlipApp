...
1. **No eligible device is seen at all.** `manual_mode.prompt_after_attempts` times over (seeded to 3). The cube is somewhere else, and trying again is exactly the right response until it isn't.
...
- **Retry** resets the attempt count and runs the whole thing again from the scan. Fail again and the same dialog comes back. That loop has no limit: it repeats until the device answers or the user picks manual mode.
...
- **`reconnectAttempt` is a delay, not a count.** Its only job is picking the backoff in `scheduleReconnect()` (`min(2 * (attempt + 1), 30)` seconds). The startup attempts need their own counter rather than a second meaning bolted onto this one, plus a "has this launch ever connected?" flag, since only the never-connected case counts against the threshold.
...
| Change | Why | Decision |
|---|---|---|
| A startup-only threshold check on `reconnectAttempt`, stopping the retries and raising the retry-or-manual dialog | This is the entry point the spec calls for. Nothing compared the counter to anything, and nothing ever stopped retrying | **Built.** `ManualModeOffer` holds the count and the has-ever-connected flag, `AppState.shouldAttemptConnection` is the gate the retry and wake paths read, and `ApplicationDelegate.offerManualMode()` runs the alert. Threshold from `manual_mode.prompt_after_attempts` |
| `ApplicationDelegate.device` becomes a reassignable `var`, swapped to `MockTimeFlipDevice` on entering manual mode | This is the actual substitution the whole approach rests on | **Built**, but through `startManualSession()` rather than `startDeviceEvents()`. That path is connect-and-log-in and would have stamped `connection.last_connection` for a device never reached, synced LED and face colours to hardware that does not exist, and stood up `MockEventHTTPServer` on an ordinary user's machine. What manual mode needs is the ingestor and the event stream, nothing else. Nothing swaps back: the only way out of the mode is a relaunch |
...
| How manual mode ends | The spec says ending needs to be at least as deliberate as entering | **Built** by construction: quit and restart the app. Nothing else ends it, because nothing else is looking. See [Entering and leaving it](#entering-and-leaving-it) for why auto-exit on the next connect was rejected |
...
| What the menu bar **does** while in manual mode | The status item is split down the middle, and the right half's pause and lock have no device to act on | **Built**, and in the end by doing nothing: manual mode is not connected, so it takes the existing no-device answer, both halves opening the menu. That is also what it wants, its stop control being the play/pause on the Faces tab and the menu being where Quit lives, which matters more here than anywhere else since quitting is the only way *out* of the mode. Two special cases were written and then removed -- the left half opening Settings (wrong: it put that single exit behind knowing which half to click) and a manual branch outranking the low-battery route (unnecessary: the offer only fires on a launch that never connected, so there is no battery reading to blink). `MenuBarClickRouter` carries the rules, out of the AppKit handler no test could call |
...
| What the menu bar **shows** while in manual mode | Today's UI reflects device state, and the menu bar read it so literally that manual mode showed a bare app name and nothing else: the display was gated on having reached a *cube* this session, and `enterManualMode` sets `.disconnected`, which tore it down outright | **Built**, and the answer is that nothing changes -- a manual session shows the same category, icon, duration and colours a connected cube does, because its reading is as current as one gets. `MenuBarLiveDisplay` holds the three questions that had been read straight off `isPaired`/`connectionStatus`: whether there is anything to draw, whether it is current enough for the live colours, and whether a `.disconnected` should clear it (in manual mode it must not, being the deliberate absence of a connection rather than a lost one) |
...
| What the **Device tab** shows while in manual mode | Its fields are all device state -- battery, auto-pause, LED, lock -- and none of it exists for a virtual device | Not fleshed out |
