...
There is already one lever for this, and it is the right one. `AppState.shouldMaintainConnection` is documented as "whether the app should be trying to reach a device at all", and it is the guard on all four entry points: the two in the wake handler, the drop handler (`ApplicationDelegate.swift:885`) and the backoff retry itself (`scheduleReconnect()`). Both new states are cases of "no": the dialog is open and waiting for an answer, and manual mode has been chosen for this launch. Feeding them into that property stops every path at once, with no new guard at any individual call site and no chance of missing one.
