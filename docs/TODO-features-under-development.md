...
Every row below is built unless it says otherwise; the two that are not are collected in [What is still open](#what-is-still-open). Kept in roughly dependency order, with each row's Decision saying what was actually done rather than what was planned -- several were built twice, and where the first attempt was wrong the row says so, since that is the part worth not repeating.
...
Two rows in the table above are unbuilt, neither blocking:
...
- **The optional guard refusing a manual write while connected.** Still optional, and still resting on the discipline described above rather than on anything the app checks.
