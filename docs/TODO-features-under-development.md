...
It also decides what **must not** be persisted. Manual mode is per-launch state, held in memory and nowhere else. A stored flag would survive the relaunch that is the only way out of it, leaving a user who had to restart to get their cube back still in manual mode with no exit at all. There is now no `setting` row for manual mode at all, which removes the obvious place to put one by mistake.
...
| (Optional) a guard refusing a manual write while `AppState.isConnected` is true | Would turn the never-both assumption into something the app checks | **Not needed, and dropped.** The state it guards against cannot occur: manual mode is entered for the rest of the launch and never left, so a manual write and a live connection cannot coexist. That is held by three separate things, two of them built for other reasons. `AppState.shouldAttemptConnection` is false from the moment manual mode is chosen, and all three attempt sites read it, so no new connect can start. Of the two paths that can reach `.connected`, one is guarded by `connectionStatus == .reconnecting` and manual mode sets `.disconnected`, and the other is `confirmConnected`, reached only while `awaitingInitialStatus` is true, which `startManualSession` deliberately never sets. A guard here would be checking for something none of those allow |
...
One row in the table above is unbuilt, and it is not blocking:

- **A `ConnectionStatus` case for "connected, to the stand-in".** Manual mode currently reports `.disconnected`, which is true -- there is no cube -- but it means every reader has to be told about manual mode separately, and three now are (`MenuBarLiveDisplay`, `MenuBarClickRouter`, `SettingsTabRules`). A case of its own would let them ask one question instead, at the cost of auditing every existing `switch`.

Two smaller things worth recording as debts rather than plans:
