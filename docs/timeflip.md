...
- **Commands with no read-back defined at all**: 0x09 (LED brightness), 0x0A (blink interval), 0x11 (face color), 0x15 (device name). The vendor spec defines no command that reads these back — the write-ack is genuinely the only confirmation available for these four. 0x15 is the strongest case of the four and the one measured rather than merely absent from the spec: the device never updates the command result characteristic for it at all (finding 2, `timeflip2-firmware-observations.md`), so even the vendor's `[cmd, 0x02]` never arrives. What confirms a rename is the *next connection* reporting the GAP name through `peripheralDidUpdateName`, which is a different mechanism from a read-back and cannot be asked for. For the LED pair and the face colours the app is therefore the system of record: what it last sent is the only account of what the device holds, and the device asks for a value back via the system-state sync-required codes when it has lost one (see §4 and *Face colours* below). The name is the exception among the four (the device does report it, just never in answer to the write), so there the app follows the device rather than standing in for it.
