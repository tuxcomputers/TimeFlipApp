...
- On startup: derive the resume position from `device_event` (the highest finalised `event_number` in the device's current counter generation, see `AppDataStore.latestCommittedDeviceEventNumber()`), fetch history starting at that `+1`, **withhold the last frame**, write all prior frames to `device_event`, and use the withheld frame to set menu/UI state. There is no stored cursor: a saved high-water mark can't follow the device's counter back down through a factory reset.

  **The derived position does not, on its own, survive a reset either, and it is worth being precise about why.** A reset restarts the device's counter at 1, and the generation boundary is found by looking for the counter going *backwards* between two stored rows. Straight after a reset no post-reset row exists, so nothing looks backwards, the whole table reads as one generation, and a device that had reached 45 still derives 45. The fetch then starts at 46, the device only holds 1 upward, nothing comes back, no row is written, and the boundary that would have corrected the position never gets created. Chicken and egg, and silent: reproduced in `Tests/TimeFlipAppTests/Workflows/W08-post-reset-production-resume.swift`.

  **What actually recovers is a separate check, and it is a live read rather than anything derived.** `HistoryIngestor.refreshHistory` asks the device for its own last event number and compares: lower than what this app holds means the cube was reset, so the cursors are cleared and the stream is re-read from 0. That, not the generation logic, is what makes a reset recoverable.
...
- On live face/pause events: re-fetch history from the cursor, write all but the last frame to `device_event`, and use the last for UI so repeated refreshes pick up duration/paused updates on the same event number.
- Cursor advancement:
