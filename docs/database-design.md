...
- `debug` = `{"enabled":true,"to_file":false,"directory":"~/Documents/Facet"}` — **a placeholder row: nothing reads any of it.**
  - `enabled` — intended to gate debug logging without a rebuild. Today the only gate is the compile-time `DeveloperMode.isDeveloperMode`, which is checked once when `DebugLog` is constructed, so a build without the dev flag has no logger at all rather than a logger that returns early. One gate is enough while the only audience is a developer with a terminal open.
