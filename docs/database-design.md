...
- `manual_mode` = `{"prompt_after_attempts":3}` — **manual mode**: driving timing from the app instead of from a cube that isn't there (see [TODO: features under development](TODO-features-under-development.md), Manual mode). `prompt_after_attempts` is how many consecutive failed reconnect attempts to an already-paired device pass before the app offers it.
  - A threshold on a counter that already exists — `ApplicationDelegate.reconnectAttempt`, incremented in `scheduleReconnect()` and reset to 0 on any successful reconnect — rather than new machinery.
  - **Startup only.** The app tries this many times, stops, and asks whether to retry or switch to manual mode; retry resets the count and runs the same number again, looping until the device answers or manual mode is chosen. Once a launch has connected even once the question is settled for that session, and a drop after that reconnects on the capped backoff indefinitely with no prompt. Seeded to 3, about twelve seconds in at the 2/4/6-second early backoff, far enough that a cube waking up or passing briefly out of range doesn't raise the offer.
  - Clamped 1-20 on load (`TimeFlipConstants`): 0 would offer manual mode before a single attempt had been made, and 20 is roughly six and a half minutes at the capped backoff, past which nobody would still be waiting to be asked.
  - Read by `AppDataStore.loadManualModePromptAfterAttempts()`. No `save` counterpart, since no control edits the value.
  - **The row holds the trigger only.** Whether manual mode is currently *on* is deliberately stored nowhere: it is per-launch, in-memory state. Quitting and restarting the app is the only way out of the mode, so a persisted flag would outlive the very restart meant to end it. See [TODO: features under development](TODO-features-under-development.md), Manual mode, for why the mode refuses to end itself on a reconnect.
