...
| | |
|---|---|
| `swift test` | **Runs. 873 tests, 0 failures, 47s** (2026-09-07) -- 540 under XCTest in 1.9s and 333 under swift-testing in 45s. That is 52 suites, out of 103 files of which 48 are excluded by name in `Package.swift`, every one of them for needing AppKit, CoreBluetooth or a `FacetApp` type |
...
**What the 873 are and are not.** They are the rules, the stores, the database layer and the device
